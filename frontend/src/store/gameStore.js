...
export const useGameStore = create(
  persist(
    (set, get) => ({
      // ========== CHARACTER SYSTEM ==========
      character: null, // Will hold the full character object from CharacterCreation
      hasCreatedCharacter: false,
      
      // Set character from CharacterCreation
      setCharacter: (characterData) => set({
        character: characterData,
        hasCreatedCharacter: true,
        username: characterData.name || 'Pirate'
      }),
      
      // Update character stats/progress
      updateCharacter: (updates) => set((state) => ({
        character: state.character ? { ...state.character, ...updates } : null
      })),
      
      // Add XP to character
      addCharacterXP: (amount) => set((state) => {
        if (!state.character) return {};
        const newXP = (state.character.experience || 0) + amount;
      ...
      addCharacterGold: (amount) => set((state) => {
        if (!state.character) return {};
        return {
          character: {
            ...state.character,
            gold: Math.max(0, (state.character.gold || 0) + amount)
          }
        };
      }),
