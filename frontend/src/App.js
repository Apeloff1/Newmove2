...
import { AchievementUnlockAnimation as EnhancedAchievementUnlock, EnhancedAchievementList } from './components/EnhancedAchievementSystem';
import EnhancedLeaderboard from './components/EnhancedLeaderboard';
import CraftingSystem from './components/CraftingSystem';
import EnhancedCookingSystem from './components/EnhancedCookingSystem';
import { calculateDrops, IngredientDropAnimation, IngredientPanel, useIngredientInventory } from './components/IngredientDropSystem';
import { BonusProvider, useBonuses, BonusDisplay } from './components/CrossSystemBonuses';
import { AchievementNotificationManager, useAchievementTracker } from './components/AchievementNotifications';
...
import './App.css';
import './pirateTheme.css';

// ========== NEW SYSTEM INTEGRATIONS (TODO LIST COMPLETE) ==========
import { FishingJournal, createJournalEntry, FISH_COLLECTIONS } from './components/FishingJournal';
import { RecipeDiscoveryNotification, RecipeDiscoveryPanel, useRecipeDiscovery } from './components/RecipeDiscoverySystem';
import { TitlesPanel, TitleBadge, useTitleManager } from './components/PlayerTitles';
import { SeasonalEventsPanel, SeasonalEventBanner, getCurrentSeason as getSeasonFromEvents, getSeasonalEvent } from './components/SeasonalEvents';
import { getRandomPirateQuote, getCurrentSeasonalContent, WORLD_LORE, LEGENDARY_PIRATES } from './lib/pirateLore';

// ========== ADVENTURE MODE ==========
import AdventureMode from './components/AdventureMode';
import { OpenWorldHub, StoreInterface } from './components/OpenWorldHub';
...
const SettingsPanel = ({ store, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-backdrop bg-black/80">
      <div className="glass-panel rounded-3xl p-8 w-full max-w-md animate-modal border-2 border-white/20">
        ...
        <div className="space-y-6 mb-8">
          ...
          <div className="flex items-center justify-between">
            <span className="text-white flex items-center gap-2">📳 Vibration</span>
            <button onClick={() => store.setVibrationEnabled && store.setVibrationEnabled(!store.vibrationEnabled)}
              className={`w-16 h-9 rounded-full transition-all duration-300 ${store.vibrationEnabled ? 'bg-green-500' : 'bg-white/20'}`}>
              <div className={`w-7 h-7 bg-white rounded-full shadow-lg transition-transform duration-300 ${store.vibrationEnabled ? 'translate-x-8' : 'translate-x-1'}`} />
            </button>
          </div>
...
function App() {
  ...
  const [showEnhancedSkillTree, setShowEnhancedSkillTree] = useState(false);
  const [showEnhancedCooking, setShowEnhancedCooking] = useState(false);
  ...
  const [ingredientDrops, setIngredientDrops] = useState(null);
  const { addIngredients, inventory: ingredientInventory } = useIngredientInventory();
  ...
  // ========== NEW INTEGRATED SYSTEMS (TODO LIST) ==========
  ...
  const [pirateQuote, setPirateQuote] = useState(null);
  
  // Hook integrations for new systems
  const { onFishCaught: trackDiscovery, pendingNotification, handleNotificationComplete, discoveries } = useRecipeDiscovery();
  const { getEquippedTitle, checkTitleUnlocks, equippedTitle } = useTitleManager();
  ...
  const catchFish = async (fish, finalTension) => {
    ...
    // Calculate and show ingredient drops
    const conditions = {
      perfect,
      combo: store.combo,
      night: store.timeOfDay === 'night',
    };
    const drops = calculateDrops(fish, conditions);
    if (drops.length > 0) {
      addIngredients(drops);
      setTimeout(() => {
        setIngredientDrops(drops);
      }, 500);
    }
    
    // ========== NEW INTEGRATED SYSTEMS (TODO LIST COMPLETE) ==========
    
    // Log to fishing journal
    try {
      createJournalEntry(fish, perfect, points, store.combo, drops);
    } catch (e) {
      console.log('Journal entry skipped');
    }
    
    // Track for recipe discovery
    try {
      trackDiscovery(fish, perfect, store.combo, store.timeOfDay, store.weather, currentSeason, store.currentStage);
    } catch (e) {
      console.log('Discovery tracking skipped');
    }
    ...
    try {
      ...
      console.log('Title check skipped');
    }
  ...
  // Menu Screen - Simplified Open World Design
  if (store.gameState === 'menu') {
    return (
      <div className="h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden" 
        ...
        <div className="flex flex-col items-center max-w-md w-full">
          ...
          <div className={`relative z-20 text-center transition-all duration-1000 ${menuAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
            <div className="relative inline-block mb-4">
              ...
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold font-pixel gradient-text logo-glow tracking-wider relative">
                GO FISH!
              </h1>
            </div>
            <p className="text-amber-200/60 text-sm font-bold tracking-widest">OPEN WORLD PIRATE ADVENTURE</p>
          </div>
          ...
          {/* SIMPLIFIED MENU - Just Adventure Mode + Settings */}
          <div className={`space-y-4 w-full relative z-20 transition-all duration-1000 delay-500 ${menuAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            
            {/* ============ ADVENTURE MODE - THE MAIN BUTTON ============ */}
            <button 
              onClick={() => { retroSounds.select(); setShowAdventureMode(true); }}
              className="w-full h-20 rounded-2xl font-bold text-white text-xl flex items-center justify-center gap-4 bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 hover:from-amber-500 hover:via-yellow-400 hover:to-amber-500 border-3 border-yellow-300/70 shadow-2xl shadow-amber-500/40 transition-all relative overflow-hidden group"
              data-testid="adventure-mode-button"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
              <span className="text-4xl group-hover:scale-110 transition-transform">🏴‍☠️</span>
              <div className="text-left">
                <div className="font-pixel text-lg">ENTER WORLD</div>
                <div className="text-xs opacity-80 font-normal">125+ NPCs • 5 Regions • Full Story</div>
              </div>
            </button>
            
            {/* Quick Play - For those who just want to fish */}
            <button 
              onClick={() => { retroSounds.select(); store.setGameState('stage_select'); }}
              className="w-full h-14 rounded-2xl font-bold text-white text-lg flex items-center justify-center gap-3 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 border-2 border-cyan-400/50 shadow-lg shadow-cyan-500/30 transition-all"
              data-testid="quick-play-button"
            >
              <span className="text-2xl">🎣</span>
              <span>QUICK FISH</span>
              <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded-full">Classic Mode</span>
            </button>
            
            {/* Bottom Row - Settings & More */}
            <div className="flex gap-3">
              <button 
                onClick={() => { retroSounds.select(); setShowSettings(true); }}
                className="flex-1 h-12 rounded-xl font-bold text-white flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 border border-white/20 transition-all"
          ...
          <div className="mt-8 text-center text-white/30 text-xs">
            <p>v2.0 • Open World Edition</p>
            <p className="text-[10px] mt-1">All game modes accessible within Adventure</p>
          </div>
        </div>
        
        {/* Decorative elements */}
        <div className="absolute bottom-10 left-10 text-6xl opacity-20 animate-float">⚓</div>
        <div className="absolute top-20 right-10 text-4xl opacity-20 animate-float" style={{animationDelay: '1s'}}>🗺️</div>
        
        {/* MODALS - Only Settings and Leaderboard from main menu */}
        {showSettings && (
          <SettingsPanel store={store} onClose={() => setShowSettings(false)} />
        )}
        ...
        {/* Adventure Mode */}
        {showAdventureMode && (
          <AdventureMode onClose={() => setShowAdventureMode(false)} />
        )}
      </div>
    );
  }
